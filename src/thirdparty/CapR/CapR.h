...
class CapR{
 public:
  CapR(string input_file, string output_file, int maximal_span){
    ...
    explicit CapR(int maximal_span){
        _input_file_name = "";
        _output_file_name = "";
        _maximal_span = maximal_span;
        _seq_length = 0;
  ...
  std::string Run_for_RaptRanker(int fwd_primer_size, int rev_primer_size, string &sequence);
  void calc_secondary_structure_profile_for_RaptRanker(int fwd_primer_size, int rev_primer_size, string &profile);
