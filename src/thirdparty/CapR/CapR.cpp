...
std::string CapR::Run_for_RaptRanker(int fwd_primer_size, int rev_primer_size, string &sequence) {
    std::string profile;

    Initiallize(sequence);
    CalcInsideVariable();
    CalcOutsideVariable();
    calc_secondary_structure_profile_for_RaptRanker(fwd_primer_size, rev_primer_size, profile);

    return(profile);
}

void CapR::calc_secondary_structure_profile_for_RaptRanker(int fwd_primer_size, int rev_primer_size, string &profile) {
    ...
    for (int i = fwd_primer_size, n = _seq_length - rev_primer_size; i < n; ++i) {
        strstream << bulge_probability[i]       << " "
                  << exterior_probability[i]    << " "
                  << hairpin_probability[i]     << " "
                  << internal_probability[i]    << " "
                  << multi_probability[i]       << " "
                  << stem_probability[i]        << "\n";
    }

    profile = strstream.str();
}
